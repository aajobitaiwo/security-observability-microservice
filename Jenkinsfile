...
pipeline{
    ...
    stages {
        ...
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image aajobitaiwo/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.20.205.25 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.146.176.249:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#aajt-devsecops-cicd-alert', //update and provide your channel name
